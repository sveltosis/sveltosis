...
watch([button0], ([button0]) => {
  if (!button0 && actionHandler0.value) {
    actionHandler0.value?.destroy();
    actionHandler0.value = null;
  } else if (button0 && !actionHandler0.value) {
    if (button0.value) {
      actionHandler0.value = onClick(button0, buttonText);
    }
  }
}),
  watch([buttonText], ([buttonText]) => {
    actionHandler0.value?.update(buttonText);
  });
